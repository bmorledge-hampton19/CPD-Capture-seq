...
print "What is the value for scaling the data (divided by)\n";
my $norm = <STDIN>;
chomp $norm;

print "What is the value for centering the data (Subtract by)\n";
my $center = <STDIN>;
chomp $center;

$clusterout =~ s/\.txt/_scal${norm}_center${center}\.txt/ || die "file not .txt file\n";
open ( CLUSTER, "$clusterfile" ) || die "Couldn't open file $clusterfile\n";

open ( OUT, ">$clusterout" ) || die "Couldn't open file $clusterout\n";

if ( $clusterfile =~ /_centered/ || $clusterfile =~ /_center/ || $clusterfile =~ /_norm/ )
{
        ;
}
else
